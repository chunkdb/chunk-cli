...
func TestValidateCommandArgs(t *testing.T) {
	cases := []struct {
		...
		{name: "chunkbinstate passthrough", cmd: "chunkbinstate", args: []string{"--out", "x", "1", "2"}, wantErr: false},
		...
		{name: "chunkcas ok", cmd: "chunkcas", args: []string{"0", "0", "42", "0101|1010"}, wantErr: false},
		{name: "chunkcas bad version", cmd: "chunkcas", args: []string{"0", "0", "-1", "0101|1010"}, wantErr: true},
		{name: "chunkcas bad state", cmd: "chunkcas", args: []string{"0", "0", "42", "01011010"}, wantErr: true},
		{name: "chunkbatch set ok", cmd: "chunkbatch", args: []string{"0", "0", "-", "SET", "1", "2", "0101"}, wantErr: false},
		{name: "chunkbatch versioned mixed ok", cmd: "chunkbatch", args: []string{"0", "0", "7", "SET", "1", "2", "0101", "UNSET", "3", "4"}, wantErr: false},
